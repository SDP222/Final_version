...
public class Window extends JPanel {
    ...
    public void update() {
        if (Database.getDataPlayer()[0] == -1) {
            ...
            int ch= (int) ((tm-Database.time)/1000);
            JOptionPane.showMessageDialog(null, "CONGRATULATION!\n" +
                    "You end game in "+ch+" sec!");
            ...
            /*
            try {
         	  File x = paramW.getScore();
                  Scanner sc = new Scanner(x); 
                  if (Integer.parseInt(sc.nextLine()) > ch)
                   {
                	 paramW.setScore(ch);
                   }
            } catch (IOException e){
